...
def _create_entry(img, question, answer,gender):
    if None!=answer:
        answer.pop('image_id')
...
def _load_dataset(dataroot, name, img_id2val, label2ans):
    ...
    if 'train' in name:
        #questions=json.load(open("./data/gender_evaluation_questions.json",'rb')) 
        #questions=json.load(open("./data/gender_train_questions.json",'rb'))    
        #questions=cPickle.load(open("./data/train_questions_gender_sorted_sample.pkl",'rb'))
        questions = sorted(json.load(open(question_path))['questions'],
                        key=lambda x: x['question_id'])
    elif 'val' in name:
        #questions=cPickle.load(open("./data/val_questions_gender_sorted_sample.pkl",'rb'))
        #questions=json.load(open("./data/gender_val_questions.json",'rb')) 
        questions = sorted(json.load(open(question_path))['questions'],
                        key=lambda x: x['question_id'])
    else:
        questions = sorted(json.load(open(question_path))['questions'],
                        key=lambda x: x['question_id'])
    gender_label=json.load(open("data/gender_label.json",'r'))
    #gender_label=json.load(open("data/gender_label_evaluation.json",'r'))

    if 'test'!=name[:4]: # train, val
        if 'train' in name:
            #answers=json.load(open("./data/gender_evaluation_answers.json",'rb'))
            #answers=cPickle.load(open("./data/train_target_gender_sorted_sample.pkl",'rb'))
            #answers=json.load(open("./data/gender_train_answers.json",'rb'))
            answer_path = os.path.join(dataroot, 'cache', '%s_target.pkl' % name)
            answers = cPickle.load(open(answer_path, 'rb'))
            answers = sorted(answers, key=lambda x: x['question_id'])

        elif 'val' in name:
            #answers=json.load(open("./data/gender_val_answers.json",'rb'))
            #answers=cPickle.load(open("./data/val_target_gender_sorted_sample.pkl",'rb'))
            answer_path = os.path.join(dataroot, 'cache', '%s_target.pkl' % name)
            answers = cPickle.load(open(answer_path, 'rb'))
            answers = sorted(answers, key=lambda x: x['question_id'])
        else:
        ...
        for question, answer in zip(questions, answers):
            ...
            gender=gender_label[str(question['question_id'])]
            if not COUNTING_ONLY or is_howmany(question['question'], answer, label2ans):
                entries.append(_create_entry(img_id2val[img_id], question, answer,gender))
    else: # test2015
        entries = []
...
class VQAFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if None!=answer:
            ...
            gender_target=torch.zeros(3)
            gender=torch.LongTensor([gender])
            
            gender_target=gender_target.scatter_(0,gender,1)
            
        
            return features, spatials, question, target,entry['question'],entry['image_id'],gender_target,gender
        else:
