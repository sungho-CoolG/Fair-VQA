...
class BanModel(nn.Module):
    def __init__(self, dataset, w_emb, q_emb, v_att,q_att, b_net, q_prj, c_prj, q_net,v_net,classifier, counter, op, glimpse):
    ...
    def forward(self, v, b, q, labels):
        ...
        '''
        v_max=torch.zeros(1,2048).cuda()
        max_att=att.argmax(1)
      
        for i in range(len(v)):
            v_max=torch.cat((v_max,v[i][max_att[i]]),0)
        v_max=v_max[1:]
        '''
        ...
        #vqa_logits=self.classifier2(v_repr*q_repr)
        #import pdb;pdb.set_trace()
        #import pdb;pdb.set_trace()
        #logits=logits.softmax(1)
        return logits,logits, att
...
def build_ban(dataset, num_hid, op='', gamma=4, task='vqa'):
    ...
    if task == 'vqa':
        ...
        return BanModel(dataset, w_emb, q_emb, v_att,q_att, b_net, q_prj, c_prj,q_net,v_net, classifier, counter, op, gamma)
    elif task == 'flickr':
