...
def train(model, train_loader, eval_loader, num_epochs, output, opt=None, s_epoch=0):
    ...
    lr_decay_step = 2
    lr_decay_rate = 0.01
    lr_decay_epochs = range(8,50,lr_decay_step) if eval_loader is not None else range(10,20,lr_decay_step)
        ...
        for j in range(len(v)):
            ...
            if gender[j]==0:
                ...
                if visual_pred[j].argmax()==gender[j].cuda():
                    ...
                    fnt=ImageFont.truetype('Pillow/Tests/fonts/FreeMono.ttf',int(width/30))
                    ...
                    for k in range(3):
                        ...
                        text.text((leftx,lefty),str(round(topk_att[j][k].item(),3)),font=fnt,fill=(255,0,0,0))

                        draw.line(coord,'red',width=3)
                    import pdb;pdb.set_trace()
                    if '/' not in str(ques[j]):
                        img.save("./gender/epoch5/gender_qatt_w_x/"+str(ques[j])+'_'+str(gender[j].item())+'_'+str(visual_pred[j].argmax().item())+".jpg")
                ...
                if visual_pred[j].argmax()==gender[j].cuda():
                    ...
                    for k in range(3):
                        ...
                        draw.line(coord,'red',width=3)
                    if '/' not in str(ques[j]):
                        img.save("./gender/epoch5/gender_qatt_m_x/"+str(ques[j])+'_'+str(gender[j].item())+'_'+str(visual_pred[j].argmax().item())+".jpg")
                    '''
            else:
                #if visual_pred[j].argmax()==0 or visual_pred[j].argmax()==1:
                other=other+1
