...
@torch.no_grad()
def evaluate(model,model_base, dataloader):
    score = 0
    score_vqa=0
    upper_bound = 0
    num_data = 0
    ...
    for i, (v, b, q, a,ques,im,g,gender) in enumerate(dataloader):
        ...
        q = q.cuda()
        gender=gender.squeeze(1)
        visual_pred, _,att = model(v, b, q, None)
        _,vqa_pred,att = model_base(v, b, q, None)
        #batch_score=torch.eq(visual_pred.argmax(1),gender.cuda()).sum()
       ...
       #import pdb;pdb.set_trace()
        ...
        gender_score=torch.zeros(len(v),3129).cuda()
        for i in range(len(v)):
        
            if int(visual_pred[i].argmax())==0 :
                for j in [1441,3087,564,2810,1065,1174]:
                    
                    gender_score[i][j]=visual_pred[i][0]
            if int(visual_pred[i].argmax())==1:
                for j in [1968,392,2456,448,2081]:
                    
                    gender_score[i][j]=visual_pred[i][1]
        vqa_pred=vqa_pred+gender_score*3
