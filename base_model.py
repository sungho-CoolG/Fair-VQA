...
class BanModel(nn.Module):
    def __init__(self, dataset, w_emb, q_emb, v_att, b_net, q_prj, c_prj, q_net,v_net,classifier, counter, op, glimpse):
...
def build_ban(dataset, num_hid, op='', gamma=4, task='vqa'):
    ...
    if task == 'vqa':
        ...
        return BanModel(dataset, w_emb, q_emb, v_att, b_net, q_prj, c_prj,q_net,v_net, classifier, counter, op, gamma)
    elif task == 'flickr':
