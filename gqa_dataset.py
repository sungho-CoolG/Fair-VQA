...
def _create_entry(img, question, question_id, answer):
    if None!=answer:
        answer.pop('image_id')
...
def _load_dataset(dataroot, name, img_id2val, label2ans, ans2label):
    """Load entries
    ...
    questions = json.load(open('/media/cvpr-pu/4TB_1/sungho1/ban-vp-vqatest/data/GVQ/question/train_balanced_questions.json'))
    #TODO question 경로 바꾸기

    if 'test'!=name[:4]: # train, val
        entries = []
        for question_id in questions:
            answer={}
            img_id = questions[question_id]['imageId']
            answer['question_id'] = question_id
            answer['image_id']=img_id
            answer['labels'] = []
            answer['labels'].append(ans2label[questions[question_id][answer]])
            answer['scores']=[]
            answer['scores'].append(1.0)
            if not COUNTING_ONLY or is_howmany(questions[question_id]['question'], answer, label2ans):
                entries.append(_create_entry(img_id2val[img_id], questions[question_id], question_id, answer))
    else: # test2015
        entries = []
        for question_id in questions:
            img_id = questions[question_id]['image_id']
            if not COUNTING_ONLY or is_howmany(questions[question_id]['question'], None, None):
                entries.append(_create_entry(img_id2val[img_id], questions[question_id], question_id, None))
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data', adaptive=False):
        ...
        self.img_id2idx = json.load(open("/media/cvpr-pu/4TB_1/sungho1/ban-vp-vqatest/data/GVQ/image/objects/gqa_objects_info.json",'r'))

        print('loading features from h5 file')
        hf_list=[]
        self.hf_num = 16
        for i in range(self.hf_num):
            hf_path = os.path.join(dataroot, '/media/cvpr-pu/4TB_1/sungho1/ban-vp-vqatest/data/GVQ/image/objects/gqa_objects_%s.h5' % (i))
            hf_list.append(h5py.File(hf_path, 'r'))

        for hf in hf_list:
            self.features.append(hf.get('features'))
            self.spatials.append(hf.get('bboxes'))

        # with h5py.File(h5_path, 'r') as hf:
        #     self.features = np.array(hf.get('features'))
        #     self.spatials = np.array(hf.get('bboxes'))
        #     if self.adaptive:
        #         self.pos_boxes = np.array(hf.get('pos_boxes'))

        self.entries = _load_dataset(dataroot, name, self.img_id2idx, self.label2ans, self.ans2label)
        self.tokenize()
        self.tensorize()
        self.v_dim = 2048
        self.s_dim = 4
    ...
    def tensorize(self):
        for i in range(self.hf_num):
            self.features[i] = torch.from_numpy(self.features[i])
            self.spatials[i] = torch.from_numpy(self.spatials[i])
...
#TODO
    def __getitem__(self, index):
        ...
        file_num = entry['image_file']
        obj_num = entry['obj']
        img_idx = entry['image']
        features = self.features[file_num][img_idx][0:obj_num-1, :]
        spatials = self.spatials[file_num][img_idx][0:obj_num-1, :]
        ...
        if None!=answer:
            ...
            return features, spatials, question, target, entry['question']
        else:
